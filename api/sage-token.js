// Create Supabase client using global fetch
...
const supabase = {
  from: (table) => ({
    ...
    select: () => ({
      eq: (column, value) => ({
        single: async () => {
          const response = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${column}=eq.${value}&select=*`, {
            headers: {
              'apikey': SUPABASE_KEY,
              'Authorization': `Bearer ${SUPABASE_KEY}`,
            }
...
async function getStoredTokens() {
  ...
  tokens.expires_at = new Date(data.expires_at);
...
export default async function handler(req, res) {
  ...
  if (storedTokens?.refreshToken || tokens.refresh_token) {
    try {
      const response = await fetch('https://oauth.accounting.sage.com/token', {
        ...
        body: new URLSearchParams({
          ...
          refresh_token: storedTokens?.refreshToken || tokens.refresh_token
        })
      });
      ...
      await storeTokens(
        data.access_token,
        data.refresh_token || storedTokens?.refreshToken || tokens.refresh_token,
